...
# Canonical JobSpy board registry used by the API and UI.
# Keep this aligned with the boards supported by python-jobspy in ApplyPilot.
JOBSPY_BOARDS = [
    {"id": "indeed", "name": "Indeed", "type": "search"},
    {"id": "linkedin", "name": "LinkedIn", "type": "search"},
    {"id": "glassdoor", "name": "Glassdoor", "type": "search"},
    {"id": "zip_recruiter", "name": "ZipRecruiter", "type": "search"},
    {"id": "google", "name": "Google Jobs", "type": "search"},
]


def _default_jobspy_sites() -> list[str]:
    blocked_sites, _ = config.load_blocked_sites()
    blocked = {str(site).strip().lower() for site in blocked_sites}
    available = [board["id"] for board in JOBSPY_BOARDS if board["id"] not in blocked]
    if available:
        return available
    return [board["id"] for board in JOBSPY_BOARDS]
