...
def load_site_colors() -> dict[str, str]:
    """Load optional site color mapping for dashboard rendering."""
    cfg = load_sites_config()
    raw = cfg.get("site_colors", {})
    return raw if isinstance(raw, dict) else {}


def load_site_delays() -> dict[str, float]:
    """Load optional per-site delay overrides for detail scraping."""
    cfg = load_sites_config()
    raw = cfg.get("site_delays", {})
...
def get_tier() -> int:
    ...
    Tier 2 (AI Scoring & Tailoring): + Gemini CLI
...
def check_tier(required: int, feature: str) -> None:
    ...
    """
    current = get_tier()
    if required <= 2 and current >= required:
        return
    ...
    if required == 3 and has_claude and has_chrome and current >= 2:
        return
    ...
    missing: list[str] = []
    if required >= 2 and shutil.which("gemini") is None:
        missing.append("Gemini CLI not found, install from [bold]https://github.com/google-gemini/gemini-cli[/bold]")
    if required >= 3:
        if not has_claude:
